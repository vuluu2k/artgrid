...
export default function AdminComment() {
  ...
  const {
    showConfirmDeleteComment: { show, commentId },
  ...
  const handleClose = () =>
    setShowConfirmDeleteComment({ show: false, commentId: "" });
          ...
          <Card style={{ width: "100%",overflow:'auto',maxHeight:'calc(100vh - 34px)' }} border="light">
            {products.map((product) => (
              <Row
                ...
                style={{ borderBottom: "1px solid #0091ff",margin:'0 16px',cursor:'pointer'}}
                onClick={() => setStateProductId({ productId: product._id })}
              >
                <Col xs="5">
